...
class CommentsAdmin(object):
    form_layout = (Main(
        ...
        Fieldset('Content', 'user', 'user_name', 'user_email', 'user_url',
                 'comment'),
    ),
                   Side(
                       Fieldset(
                           _('Metadata'), 'submit_date', 'ip_address',
...
if 'django.contrib.comments' in settings.INSTALLED_APPS and (get_model() is
                                                             Comment):
