...
class AjaxListPlugin(BaseAjaxPlugin):
    def get_list_display(self, list_display):
        list_fields = [
            field for field in self.request.GET.get('_fields', "").split(",")
            if field.strip() != ""
        ]
        if list_fields:
            return list_fields
    ...
    def get_result_list(self, response):
        ...
        headers = dict([(c.field_name, force_text(c.text))
...
class JsonErrorDict(ErrorDict):
    ...
    def as_json(self):
        ...
        return [{
            'id':
            self.form[k].auto_id
            if k != NON_FIELD_ERRORS else NON_FIELD_ERRORS,
            'name':
            k,
            'errors':
            v
...
class AjaxFormPlugin(BaseAjaxPlugin):
    def post_response(self, __):
        ...
        return self.render_response({
            ...
            str(new_obj),
            'change_url':
            self.admin_view.model_admin_url('change', new_obj.pk),
            'detail_url':
            self.admin_view.model_admin_url('detail', new_obj.pk)
        })
