...
class RecoverListView(BaseReversionView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return TemplateResponse(
            request, self.recover_list_template
            or self.get_template_list("views/recover_list.html"), context)
...
class RevisionListView(BaseReversionView):
    ...
    def get_response(self):
        ...
        return TemplateResponse(
            self.request, self.object_history_template
            or self.get_template_list('views/model_history.html'), context)
    ...
    def post(self, request, object_id, *args, **kwargs):
        ...
        return TemplateResponse(
            self.request, self.revision_diff_template
            or self.get_template_list('views/revision_diff.html'), context)
...
class BaseRevisionView(ModelFormAdminView):
    ...
    def get_media(self):
        return super(
            BaseRevisionView,
            self).get_media() + self.vendor('xadmin.plugin.revision.js')
...
class RevisionView(BaseRevisionView):
    ...
    def get_context(self):
        ...
        context["title"] = _("Revert %s") % force_text(
    ...
    def get_response(self):
        ...
        return TemplateResponse(
            self.request, form_template
            or self.get_template_list('views/revision_form.html'), context)
...
class RecoverView(BaseRevisionView):
    ...
    def get_response(self):
        ...
        return TemplateResponse(
            self.request, form_template
            or self.get_template_list('views/recover_form.html'), context)
...
class InlineDiffField(Field):
    def render(self,
        ...
        if not instance.pk:
            return super(InlineDiffField, self).render(form, form_style,
                                                       context)
...
class InlineRevisionPlugin(BaseAdminPlugin):
    def get_related_versions(self, obj, version, formset):
        ...
        # Get the fk name.
        try:
            fk_name = formset.fk.name + '_' + formset.fk.rel.get_related_field(
            ).name
        ...
        related_versions = dict([
            ...
            model_class() == formset.model and smart_text(
                related_version.field_dict[fk_name]) == smart_text(object_id)
        ])
    ...
    def _hack_inline_formset_initial(self, revision_view, formset):
        ...
        for related_obj in formset.queryset:
            if smart_text(related_obj.pk) in related_versions:
                initial.append(
                    related_versions.pop(smart_text(
        ...
        if self.request.method == 'GET' and formset.helper and formset.helper.layout:
            ...
            fake_admin_class = type(
                str('%s%sFakeAdmin' % (self.opts.app_label,
                                       self.opts.model_name)), (object, ),
                {'model': self.model})
            for form in formset.forms:
                instance = form.instance
                if instance.pk:
